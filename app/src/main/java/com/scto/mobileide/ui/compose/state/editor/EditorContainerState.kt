...
import com.scto.mobileide.core.config.EditorSettings
...
import com.scto.mobileide.core.packages.PackageDependencyEvents
...
import com.scto.mobileide.plugin.script.api.EditorSelectionPayload
...
import com.scto.mobileide.ui.compose.components.editor.EditorToolBarState
import java.io.File
import java.net.URI
...
class EditorContainerState(
    ...
    fun syncFromManager(managerTabs: List<EditorTab>, activeTabId: String?) {
        ...
        if (previousActiveTabId != null && previousActiveTabId != currentActiveTabId) {
            releaseMobileLspForTab(previousActiveTabId)
        }
    }
    ...
    fun attachMobileLspForTab(tabId: String, file: File, textProvider: () -> String): Boolean = lspEditorManager.attachMobileLsp(
        context = context,
        file = file,
        tabId = tabId,
    ...
    fun releaseMobileLspForTab(tabId: String) {
        lspEditorManager.releaseLspEditor(tabId)
        lspStatusesByTabId.remove(tabId)
    }

    fun notifyMobileTextChanged(tabId: String, change: TextChange) {
        lspEditorManager.onMobileDocumentChanged(tabId, change)
    }
    ...
    suspend fun executeCodeAction(
        tabId: String,
        action: com.scto.mobileide.core.lsp.LspCodeActionService.CodeActionItem,
