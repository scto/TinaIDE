...
description: MobileIDE 测试、排障和质量验证指南。用于选择单元测试/仪器测试命令、定位失败原因、验证 editor/AI/plugin/database/security 等模块改动，或为文档变更做轻量检查。
---
...
./gradlew :feature:ai:testDebugUnitTest --tests "com.scto.mobileide.ai.*" --console=plain
...
./gradlew :core:editor-view:connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.scto.mobileide.core.editorview.EditorCompletionPopupInstrumentationTest,com.scto.mobileide.core.editorview.EditorSharedPopupInstrumentationTest --console=plain
```
...
- 多数 `core/*` 与 `feature/*` Android library 模块通过 `mobile.android.library` convention 自动获得 JUnit、Truth、Robolectric、MockK、coroutines-test；不要仅凭模块 `build.gradle.kts` 未写 `testImplementation` 就重复加依赖。
