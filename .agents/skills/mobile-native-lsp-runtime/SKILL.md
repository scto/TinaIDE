---
name: mobile-native-lsp-runtime
description: MobileIDE native 编译运行、Android sysroot、mobile-toolchain assets、clangd/LSP、Tree-sitter 与 PRoot 排障指南。用于处理 C/C++ 编译、clang/lld/clangd、语言服务、toolchain 包、queries/registry 或 PRoot 相关改动。
---
...
- `app/src/main/java/com/scto/mobileide/ui/compose/state/editor/EditorContainerState.kt`。
- `app/src/main/java/com/scto/mobileide/ui/compose/components/editor/MobileCodeEditorPage.kt`。
- `app/src/arm64/assets/mobile-toolchain/current.properties`。
- `app/src/x86_64/assets/mobile-toolchain/current.properties`。
- `tools/verify-mobile-toolchain-package.ps1`、`tools/sync-mobile-toolchain-assets.ps1`。

## 项目事实

- 默认编译/LSP 依赖 native mobile-toolchain + Android sysroot，不默认依赖 PRoot rootfs。
- `AndroidSysrootManager` 与 `AndroidNativeToolchainManager` 是独立管理职责，不能合并。
- toolchain assets 由 `app/src/<abi>/assets/mobile-toolchain/current.properties`、tar.xz 和 sha256 约束。
- arm64 当前记录为 `version=0.2.4`、`arch=aarch64`、`full=mobileide-toolchain-aarch64-v0.2.4-patched.tar.xz`。
- x86_64 当前记录为 `version=0.2.3`、`arch=x86_64`、`full=mobileide-toolchain-x86_64-v0.2.3.tar.xz`。
...
4. LSP 状态或 UI 变更同时检查 `LspEditorManager`、`EditorContainerState`、`MobileCodeEditorPage`。
...
pwsh ./tools/verify-mobile-toolchain-package.ps1
```
