...
description: MobileIDE 构建、Gradle、ABI、CI、发布、签名和 R8 排障指南。用于处理 assemble/compile 失败、APK 构建、版本号、GitHub Actions、toolchain assets 校验或 Release 混淆问题。
---
...
- `app` flavor 是 `arm64` 与 `x86_64`，本地默认 `mobile.devAbi=arm64`。
- 全 ABI 用 `:app:assembleDebugAllAbi`、`:app:assembleReleaseAllAbi` 或 `-Pmobile.allAbi=true`。
...
- Release 构建会备份 R8 mapping；mapping 上传默认关闭，只有显式启用 `mobile.releaseMapping.uploadEnabled=true` 才会上传。
- CI 使用 JDK 17、CMake 3.22.1、tree-sitter-cli 0.22.1，并 recursive checkout submodules。

## 复用入口

- ABI 聚合：`MobileAndroidAppAbiAggregationPlugin`。
- 版本递增：`MobileAndroidAppVersioningPlugin`。
- mapping 备份/上传：`MobileAndroidAppMappingPlugin`。
- Tree-sitter registry 和 ktlint 任务衔接：`MobileAndroidAppTreeSitterPlugin`。
- toolchain assets 完整性：`verifyMobileToolchainAssets` 相关 build-logic。
- 库模块 `consumer-rules.pro` 自动注册：`MobileAndroidLibraryPlugin`。

## 禁止事项

- 不要重复注册 `assembleDebugAllAbi` 或重新实现版本递增。
- 不要把 `syncTreeSitterQueries` 挂到常规 build；它是手动联网任务。
- 不要把 Gradle build 产物重定向到 `LOCALAPPDATA/MobileIDE/gradle-out/**`。
