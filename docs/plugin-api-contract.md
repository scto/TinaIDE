...
| `configuration` | 否 | 稳定 | 插件配置 schema；宿主在插件详情页自动生成设置 UI，并提供 `mobile.config.*` 读写。 |
...
- `contributions.menus[*].command` 现在允许两类值：
  - 宿主白名单命令
  - 当前插件已通过 `mobile.commands.register()` 注册的插件命令
...
插件可通过 `mobile.events.on("config.changed", callbackName)` 监听自身配置变化。该事件只会定向派发给配置所属插件，
不会广播其他插件的配置值。
...
`fs` 是历史兼容命名空间，当前仍保留。新插件应优先使用 `mobile.workspace.*`。
