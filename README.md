...
> 运行在 Android 设备上的 C/C++ IDE，默认使用 `native mobile-toolchain + Android sysroot`；可选提供自研 Linux distro / PRoot 环境。

[English](README_EN.md)

---

MobileIDE 是面向手机和平板的移动端 IDE。当前版本的核心变化是运行时被拆成了两层：

- 默认开发链路：内置 `native mobile-toolchain`、`Android sysroot`、native clangd/LSP
...
- 原生 C/C++ 工具链：内置 `mobile-toolchain`，提供 clang / clang++ / clangd 等能力
- Android sysroot：按 ABI 安装与校验运行时必需的头文件和库
- 智能补全：native clangd + LSP，支持诊断、跳转定义、查找引用、悬浮信息
- 自研编辑器：Mobile Editor（Compose + Canvas），支持多标签、增量高亮、会话管理
...
./gradlew -Pmobile.devAbi=x86_64 :app:assembleX86_64Debug
./gradlew -Pmobile.devAbi=x86_64 :app:installX86_64Debug
...
# 如需临时使用 Docker 预编译产物
.\gradlew.bat :app:assembleArm64Debug '-Pmobile.buildProotFromSource=false'

# 同步 mobile-toolchain / sysroot 资产到 app/src/<abi>/assets
pwsh ./tools/sync-mobile-toolchain-assets.ps1 -Abi arm64

# 刷新自研 Linux 发行版 manifest（可选，维护发行版元数据时使用）
pwsh ./tools/linux-distro/generate-linux-distro-manifest.ps1
```

当前 `mobile-toolchain` 资产由 `app/src/<abi>/assets/mobile-toolchain/current.properties` 描述，构建时会执行 `verifyMobileToolchainAssets` 校验。
...
| 编辑器 | Mobile Editor（Compose + Canvas） |
| 高亮 | Tree-sitter |
| 编译 | native mobile-toolchain |
...
MobileIDE 已决定彻底开源，后续精力会优先放在稳定维护和社区协作上。
