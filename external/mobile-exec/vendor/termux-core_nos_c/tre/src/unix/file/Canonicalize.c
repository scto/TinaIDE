...
#ifndef CANONICALIZE_LOG_TAG
#define CANONICALIZE_LOG_TAG "canonicalize"
#endif
...
char* __normalizePath(char* path, bool keepEndSeparator, bool removeDoubleDot) {
    ...
    if (nc >= 2) {
        ...
        if (result == NULL) {
            logStrerrorDebug(CANONICALIZE_LOG_TAG, "The malloc called failed for ix in 'normalizePath(%s)' with size '%zu'", path, ix_size);
            return NULL;
        }
