...
if [ "$need_rebuild_loader" = true ]; then
    ...
    # 添加 16KB 页面对齐参数到 LOADER_LDFLAGS
    make -B -j"$(nproc)" CC="${CC}" CFLAGS="${CFLAGS}" LOADER_LDFLAGS="-static -nostdlib -Wl,--build-id=none,-z,max-page-size=16384,--rosegment,-z,noexecstack" V=1 loader/loader
...
if [ "${TARGET_ARCH}" = "aarch64" ]; then
    ...
    if [ "$need_rebuild_loader32" = true ]; then
        cd "${PROOT_SRC_DIR}"
        # 添加 16KB 页面对齐参数到 LOADER_LDFLAGS-m32
        make -B -j"$(nproc)" CC="${CC}" CFLAGS="${CFLAGS}" LOADER_LDFLAGS-m32="-static -nostdlib -Wl,--build-id=none,-z,max-page-size=16384,--rosegment,-z,noexecstack" V=1 loader/loader-m32
...
echo "下一步:"
echo "  将文件复制到项目:"
echo "    libproot*.so → app/src/prebuiltProot/jniLibs/${ANDROID_ABI}/"
echo "    libtalloc.so.2 → app/src/<flavor>/assets/proot/${ANDROID_ABI}/"
echo ""
echo "  或使用 PowerShell 脚本自动复制:"
echo "    .\\build-proot.ps1 -CopyToJniLibs -CopyToAssets"
