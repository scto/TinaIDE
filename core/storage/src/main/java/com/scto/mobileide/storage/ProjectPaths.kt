 ...
 *    - config.json    - 应用配置
 *
 * 2. 公有源码目录（Documents/MobileIDE）
 *    路径：/sdcard/Documents/MobileIDE/
 *    特性：需要外部存储权限，用户可直接访问
 *    用途：
 *    - 用户源码目录（主源码）
 *
 ...
 * 核心原则：
 * - 源码目录与构建工作区彻底分离
 * - 用户源码默认放在公有 Documents/MobileIDE 目录
...
object ProjectPaths {
     ...
     * 公有源码根目录。
     *
     * 典型路径：/sdcard/Documents/MobileIDE
     */
    fun getPublicProjectsRoot(context: Context): File {
        ...
        return File(documentsDir, "MobileIDE")
    }
