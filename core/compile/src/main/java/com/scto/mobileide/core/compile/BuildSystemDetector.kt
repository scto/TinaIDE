...
object BuildSystemDetector {
     ...
     *
     * 检测顺序：
     * 1. 从项目元数据（.mobileide/project.json）读取
        ...
        return try {
            when {
                hasPluginManifest(projectRoot) -> {
                    Timber.tag(TAG).d("Detected MobileIDE plugin project")
                    BuildSystem.PLUGIN
                }
    ...
    /**
     * 检查是否是 MobileIDE 插件项目。
     *
     * 只把具有插件清单关键字段和插件特征字段的 manifest.json 识别为插件项目，
     * 避免把普通 Web/包管理 manifest 误判为 MobileIDE 插件。
     */
    private fun hasPluginManifest(projectRoot: File): Boolean {
        ...
        return runCatching {
            ...
            Timber.tag(TAG).d(throwable, "manifest.json is not a MobileIDE plugin manifest")
            false
        }
    }
