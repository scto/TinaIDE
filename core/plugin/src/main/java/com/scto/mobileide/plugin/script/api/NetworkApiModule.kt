...
import com.scto.mobileide.plugin.PluginNetworkHostRules
    ...
    override fun register(runtime: ScriptPluginRuntime, lua: Lua) {
        ...
        lua.push { L: Lua ->
            withPermission(
                ...
                if (!rt.checkNetworkLimit()) { L.pushNil(); L.push("Network request rate limit exceeded"); return@withPermission 2 }
                val url = L.getStringArg(1)
                if (url == null) { L.pushNil(); L.push("URL is required"); return@withPermission 2 }
                if (!rt.checkPermission(PluginPermission.NETWORK_UNRESTRICTED) &&
                    ...
                    L.pushNil(); L.push("Host not in whitelist: ${PluginNetworkHostRules.extractRequestHost(url)}"); return@withPermission 2
                }
                ...
                try {
                    ...
                    when (method) {
                        ...
                        else -> { L.pushNil(); L.push("Unsupported HTTP method: $method"); return@withPermission 2 }
                    }
                    val response = httpClient.newCall(requestBuilder.build()).execute()
                    L.createTable(0, 4)
                    L.push(response.code); L.setField(-2, "status")
                    L.push(response.isSuccessful); L.setField(-2, "ok")
                    L.push(response.body?.string() ?: ""); L.setField(-2, "body")
                    L.createTable(0, response.headers.size)
                    response.headers.forEach { (name, value) -> L.push(value); L.setField(-2, name) }
                    L.setField(-2, "headers")
                    response.close()
                    1
                } catch (e: Exception) {
                    Timber.w(e, "Network request failed: $url")
                    L.pushNil(); L.push("Network request failed: ${e.message}"); 2
                }
            }
        }
        ...
        lua.push { L: Lua ->
            withPermission(
                ...
                val rt = this.runtime ?: return@withPermission 0
                val url = L.getStringArg(1) ?: run { L.pushNil(); L.push("URL is required"); return@withPermission 2 }
                if (!rt.checkPermission(PluginPermission.NETWORK_UNRESTRICTED) &&
                    !PluginNetworkHostRules.isUrlAllowed(url, rt.getAllowedHosts(), normalizedAllowedHosts)
                ) {
                    L.pushNil(); L.push("Host not in whitelist: ${PluginNetworkHostRules.extractRequestHost(url)}"); return@withPermission 2
                ...
                try {
                    ...
                    L.pushNil(); L.push("GET request failed: ${e.message}"); 2
                }
            }
        }
        lua.setField(-2, "get")
        ...
        lua.push { L: Lua ->
            withPermission(
                ...
                try {
                    ...
                    response.close()
                    L.push(responseBody); 1
                } catch (e: Exception) {
                    Timber.w(e, "POST request failed: $url")
                    L.pushNil(); L.push("POST request failed: ${e.message}"); 2
                }
            }
        }
        lua.setField(-2, "post")
