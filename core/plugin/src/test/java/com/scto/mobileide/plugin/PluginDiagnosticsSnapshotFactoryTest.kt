...
import com.scto.mobileide.plugin.script.ScriptPluginState
...
class PluginDiagnosticsSnapshotFactoryTest {
    ...
    fun `create should merge recent runtime permission diagnostics from logs`() {
        ...
        val snapshot = PluginDiagnosticsSnapshotFactory.create(
            ...
            pluginLogs = listOf(
                PluginLogEntry(
                    ...
                    message = "Permission denied for mobile.fs.readFile: file.read is not granted",
                ...
                PluginLogEntry(
                    ...
                    pluginName = plugin.manifest.name,
                    level = PluginLogLevel.WARN,
                    message = "Permission denied for mobile.commands.execute: command.execute is not declared in manifest",
                    eventCode = PluginLogEventCodes.PERMISSION_DENIED,
                    attributes = mapOf(
        ...
        assertThat(report?.entries?.map { it.issue.message })
            .containsExactly(
                "Permission denied for mobile.commands.execute: command.execute is not declared in manifest",
                "Permission denied for mobile.fs.readFile: file.read is not granted",
            )
