...
import com.scto.mobileide.plugin.PluginLogEventCodes
import com.scto.mobileide.plugin.PluginLogEventKeys
...
class WorkspaceApiModuleRuntimeTest {
    ...
    fun setUp() {
        ...
        workspaceRoot = Files.createTempDirectory("mobile-workspace-api-runtime").toFile()
        logManager.clearAll()
    }
        ...
        try {
            ...
            val defineResult = runtime.execute(
                ...
                function exercise_workspace_api()
                  local ok, write_err = mobile.workspace.writeFile("notes/result.txt", "hello workspace")
                  if not ok then error(write_err) end

                  local content, read_err = mobile.workspace.readFile("notes/result.txt")
                  if content ~= "hello workspace" then error(read_err or "unexpected content") end

                  local files = mobile.workspace.findFiles("*.kt", 10)
                  return mobile.apiVersion .. "|" .. mobile.pluginId .. "|" .. content .. "|" .. table.concat(files, ",")
                end
                """.trimIndent()
            )
        ...
        try {
            ...
            val defineResult = runtime.execute(
                ...
                function read_without_permission()
                  local content, err = mobile.workspace.readFile("README.md")
