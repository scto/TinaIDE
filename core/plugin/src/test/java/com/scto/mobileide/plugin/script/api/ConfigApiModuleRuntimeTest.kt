        ...
        try {
            ...
            val defineResult = runtime.execute(
                ...
                function exercise_config_api()
                  local before = mobile.config.get("feature.enabled")
                  local ok_enabled = mobile.config.set("feature.enabled", true)
                  local ok_format = mobile.config.set("output.format", "json")
                  local bad_format, bad_format_err = mobile.config.set("output.format", "xml")
                  local fallback = mobile.config.get("missing.key", "fallback")
                  local declared_fallback = mobile.config.get("display.label", "fallback")
                  local jobs = mobile.config.get("build.jobs")
                  return tostring(before)
                    .. "|" .. tostring(ok_enabled)
                    .. "|" .. tostring(ok_format)
                    .. "|" .. tostring(bad_format)
                    .. "|" .. tostring(mobile.config.get("feature.enabled"))
                    .. "|" .. mobile.config.get("output.format")
